...
import ai.lum.odinson.{OdinsonIndexWriter, Document => OdinsonDocument, _}
...
class TestOdinsonIndexWriter extends OdinsonTest with BeforeAndAfterEach {
    ...
    "OdinsonIndexWriter" should "object should return index from config correctly" ignore {
        // get index writer
    ...
    it should "mkLuceneFields should convert Fields to lucene.Fields correctly" ignore {
        val indexWriter = getOdinsonIndexWriter()
    ...
    it should "incrementally append to a new index" ignore {
        val appendConf =
    ...
    it should "incrementally append to an existing index" in {
        ...
        // add the first doc...
        indexer.addDocument( getDocument( "alien-species" ) )
        indexer.close()

        val index : FSDirectory = FSDirectory.open( indexDir.toPath )

        // add a generic rule that should work for any document
        val extractorEngine = ExtractorEngine.fromDirectory( appendConf, index )
        val nounsRule =
            """
              |rules:
              |  - name: get_nouns
        ...
        val nounExtractor = extractorEngine.compileRuleString( nounsRule )
        val firstQueryResults = extractorEngine.extractNoState( nounExtractor )

        indexer = getOdinsonIndexWriter( appendConf )

        // add another doc
        indexer.addDocument( getDocument( "pre-european-diet" ) )
        indexer.close()

        val secondQueryResults = extractorEngine.extractNoState( nounExtractor )

        firstQueryResults.foreach( m => println( m.docId ) )
        secondQueryResults.foreach( m => println( m.docId ) )

        println( firstQueryResults == secondQueryResults )
    ...
    it should "properly dump and load relevant settings" ignore {
