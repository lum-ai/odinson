...
class TestOdinsonTest extends OdinsonTest {
  ...
  it should "convert matches to strings" in {
    ...
    val odinsonMatches: Array[OdinsonMatch] = Array(new NGramMatch(2,3), new NGramMatch(4,5), new NGramMatch(7,8))
    ...
    mkStrings(results, ee) should contain theSameElementsInOrderAs  Seq("ramen", "chopsticks", "spoon")
  }
  ...
  it should "determine if two mentions are equal" in {
    ...
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy mentionsShouldBeEqual(m1, m2)
  ...
  it should "determine if matches are equal" in {
    ...
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy matchesShouldBeEqual(eventMatch1, eventMatch2)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy matchesShouldBeEqual(eventMatch1, eventMatch3)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy matchesShouldBeEqual(eventMatch1, eventMatch4)
    ...
    val matches1shuffled: Array[OdinsonMatch] = Array(eventMatch3, eventMatch2, eventMatch4, eventMatch1)
    val matches2: Array[OdinsonMatch] = Array(eventMatch3, new NGramMatch(1, 2), eventMatch4, eventMatch1)

    noException should be thrownBy matchesShouldBeEqual(matches1, matches1)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy matchesShouldBeEqual(matches1, matches1shuffled)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy matchesShouldBeEqual(matches1, matches2)
  }
  ...
  it should "determine if named captues are equal" in {
    ...
    val nc5 = NamedCapture("name", Some("Label"), new NamedMatch(new NGramMatch(1,2),"namedmatch", Some("Label")))

    val named = Array(nc1, nc2, nc3, nc4, nc5)

    noException should be thrownBy namedCapturesShouldBeEqual(named, named)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy namedCapturesShouldBeEqual(Array(nc1), Array(nc2))
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy namedCapturesShouldBeEqual(Array(nc1), Array(nc3))
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy namedCapturesShouldBeEqual(Array(nc1), Array(nc4))
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy namedCapturesShouldBeEqual(Array(nc1), Array(nc5))
  ...
  it should "determine if argument metadata are equal" in {
    ...
    noException should be thrownBy argMetaDataShouldBeEqual(Array(arg1, arg2, arg3, arg4, arg5), Array(arg1, arg2, arg3, arg4, arg5))
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy argMetaDataShouldBeEqual(Array(arg1), Array(arg2))
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy argMetaDataShouldBeEqual(Array(arg1), Array(arg3))
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy argMetaDataShouldBeEqual(Array(arg1), Array(arg4))
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy argMetaDataShouldBeEqual(Array(arg1), Array(arg5))
  }
  ...
  it should "test mention contents" in {
    ...
    // non event with args
    val m2 = new Mention(new NGramMatch(0,1), Some("foobar"), 0, 0, 0, nullIdGetter, "FoobarRule", Map("food" -> Array(m1)))
    ...
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testMention(m3, "Jacob", Seq(Argument("food", "ramen")), ee)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testMention(m3, "John", Seq(Argument("drink", "ramen")), ee)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testMention(m3, "John", Seq(Argument("food", "candy")), ee)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testMention(m3, "John", Seq(Argument("food", "ramen"), Argument("food", "candy")), ee)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testMention(m3, "John", Seq(), ee)
    ...
    noException should be thrownBy testMention(m4, "John", Seq(Argument("food", "ramen"), Argument("food", "chopsticks")), ee)
    // diff order of args provided
    noException should be thrownBy testMention(m4, "John", Seq(Argument("food", "chopsticks"), Argument("food", "ramen")), ee)
    // repeated
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testMention(m4, "John", Seq(Argument("food", "chopsticks"), Argument("food", "ramen"), Argument("food", "ramen")), ee)
  }
  ...
  it should "test event trigger" in {
    ...
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testEventTrigger(eventMatch, 1, 2)
  ...
  it should "test event trigger string" in {
    ...
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testEventTrigger(0, eventMatch, ee, "ramen")
  ...
  it should "test  arguments by offsets" in {
    ...
    noException should be thrownBy testArguments(eventMatch, Seq(ArgumentOffsets("food", 2, 3), ArgumentOffsets("food", 4,5)))
    // diff order should be fine
    noException should be thrownBy testArguments(eventMatch, Seq(ArgumentOffsets("food", 4, 5), ArgumentOffsets("food", 2,3)))

    // diff offsets
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(eventMatch, Seq(ArgumentOffsets("food", 2, 3), ArgumentOffsets("food", 4,6)))
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(eventMatch, Seq(ArgumentOffsets("food", 1, 3), ArgumentOffsets("food", 4,5)))
    // missing one
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(eventMatch, Seq(ArgumentOffsets("food", 2, 3)))
    // missing one, other duplicated
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(eventMatch, Seq(ArgumentOffsets("food", 2, 3), ArgumentOffsets("food", 2,3)))
    // duplicated
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(eventMatch, Seq(ArgumentOffsets("food", 2, 3), ArgumentOffsets("food", 2,3), ArgumentOffsets("food", 4,5)))
  }
  ...
  it should "test arguments by content" in {
    ...
    noException should be thrownBy testArguments(0, eventMatch, Seq(Argument("food", "ramen"), Argument("food", "chopsticks")), ee)
    // diff order should be fine
    noException should be thrownBy testArguments(0, eventMatch, Seq(Argument("food", "chopsticks"), Argument("food", "ramen")), ee)

    // diff string
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(0, eventMatch, Seq(Argument("food", "ramen"), Argument("food", "spoon")), ee)
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(0, eventMatch, Seq(Argument("food", "ramen"), Argument("tool", "chopsticks")), ee)
    // missing one
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(0, eventMatch, Seq(Argument("food", "ramen")), ee)
    // missing one, other duplicated
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(0, eventMatch, Seq(Argument("food", "ramen"), Argument("food", "ramen")), ee)
    // duplicated
    an [org.scalatest.exceptions.TestFailedException] should be thrownBy testArguments(0, eventMatch, Seq(Argument("food", "ramen"), Argument("food", "ramen"), Argument("food", "chopsticks")), ee)
  }
