...
class TestEvents extends EventSpec with BeforeAndAfterAll with BeforeAndAfterEach {
  var ee: ExtractorEngine = _

  override def beforeAll(): Unit = {
    val doc = getDocument("becky-gummy-bears")
    ee = Utils.mkExtractorEngine(doc)
  }
  ...
  it should "match event with promoted entities" in {
    val pattern =
      """
    trigger = [lemma=eat]
...
"""
  ...
  it should "find event with mentions from the state when the state is populated" in {
    ...
    def localQuery(odinsonQuery: OdinsonQuery, labelOpt: Option[String] = None, nameOpt: Option[String] = None, n: Int, after: OdinsonScoreDoc, disableMatchSelector: Boolean): OdinResults = {
      ...
      val odinMentionsIterator = OdinResultsMentionIterator(labelOpt, nameOpt, odinResults, ee.mentionFactory, mruIdGetter)
