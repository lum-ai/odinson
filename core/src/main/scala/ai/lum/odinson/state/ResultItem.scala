...
case class ResultItem(
  ...
  override def toString: String = s"""${getClass.getSimpleName}(segmentDocBase = $segmentDocBase, segmentDocId = $segmentDocId, docIndex = $docIndex, label = "$label", name = "$name", odinsonMatch = $odinsonMatch)"""
