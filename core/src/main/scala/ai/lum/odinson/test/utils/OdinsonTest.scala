...
class OdinsonTest extends FlatSpec with Matchers {
    ...
    /** Constructs an ExtractorEngine with an in-memory index, and the default config, BUT with
      ...
      * @return
      */
    def extractorEngineWithConfigValue( doc : OdinsonDocument, key : String, value : Any ) : ExtractorEngine = {
    ...
    def mkMemoryExtractorEngine( config : Config, doc : OdinsonDocument ) : ExtractorEngine = {
        if ( config.apply[ String ]( "odinson.indexDir" ) == ":memory:" ) ExtractorEngine.inMemory( config, Seq( doc ) )
        else {
            ExtractorEngine.fromConfig( config )
        }
    }
