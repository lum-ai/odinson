...
// For this, we can write a display method
  def displayMention(mention: Mention): Unit = {
    ...
    val text = extractorEngine.getStringForSpan(luceneDocID, odinsonMatch)
    println()
    println("-" * 30)
    ...
    if (mention.arguments.nonEmpty) {
      println("  Args:")
      val stringified = mention.arguments.mapValues(ms => ms.map(m => (extractorEngine.getStringForSpan(luceneDocID, m.odinsonMatch), m.label)))
